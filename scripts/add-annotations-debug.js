...
async function addAnnotations() {
  ...
  const debugPath = path.join(__dirname, '..', 'public', 'debug-annotated.png');
  ...
  try {
    ...
    // Save as PNG (both debug and temp)
    const buffer = canvas.toBuffer('image/png');
    fs.writeFileSync(tempPath, buffer);
    fs.writeFileSync(debugPath, buffer); // Save debug version
    ...
    // Clean up temp file (keep debug for verification)
    ...
    console.log(`  Debug PNG saved to: ${debugPath}`);
  } catch (error) {
